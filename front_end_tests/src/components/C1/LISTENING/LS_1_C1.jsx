...
function LS_1_C1({ data, level, section, part, modelName, video, explanation, enunciado }) {
  ...
  // Render explicación con números en negrita y palabras clave
  const renderExplanation = () => {
    if (!explanation || !explanation.explanation) return null;
    let text = explanation.explanation;
    
    // Reemplazar "Extract one", "Extract two", "Extract three" con negrita
    text = text.replace(/(Extract one|Extract two|Extract three)/gi, (match) => {
      return `<strong class="font-bold text-amber-800 text-base inline-block">${match}</strong>`;
    });
          ...
          <div className="p-5">
            <div className="text-gray-700 leading-relaxed space-y-3">
              {lines.map((line, idx) => {
                ...
                // Procesar HTML especial para Extract
  ...
  // Obtener las preguntas de un extracto específico
  const getQuestionsForExtract = (extract) => {
    return data.questions.filter(q => extract.questions.includes(q.id));
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-7">
      ...
      {/* Extracts and Questions - SIN estilos azules */}
      <div className="grid grid-cols-1 gap-8">
        <div className="space-y-6">
          <form className="space-y-8">
            {data.extracts && data.extracts.map((extract) => {
              const extractQuestions = getQuestionsForExtract(extract);
              
              return (
                <div key={extract.id} className="mb-8">
                  {/* Extract description - solo texto más grande y bold */}
                  <div className="mb-10">
                    <div className="flex items-center items-center gap-2 text-6xl">
                      <span className="font-bold text-gray-800 text-2xl">Extract {extract.id}:</span>
                      <span className="text-gray-700 text-xl font-medium">{extract.description}</span>
                    </div>
                  </div>
                        ...
                        <div key={q.id} className="pb-6 border-b border-gray-100 last:border-0">
                          ...
                          <div className="grid gap-2">
                            {q.options.map((opt, idx) => {
                              ...
                              let style = "p-3 rounded-xl text-left cursor-pointer transition-all duration-200 ";
