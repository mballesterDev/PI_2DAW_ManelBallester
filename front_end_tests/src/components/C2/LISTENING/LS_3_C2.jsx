...
function LS_3_C2({ data, level, section, part, modelName, video, explanation, enunciado }) {
  ...
  const handleAnswerChange = (speakerIndex, optionValue) => {
    // Clear warning for this speaker
    setDuplicateWarning(prev => ({ ...prev, [speakerIndex]: false }));
    
    // Check if answer is already used by another speaker
  ...
  // Check if an option is selected by another speaker (for tachado)
  ...
  // Get letter for option (A, B, C, D, E, F, G, H)
  const getOptionLetter = (index) => {
    return String.fromCharCode(65 + index);
  };

  // Get the letter for a given option text
  ...
  const renderTranscript = () => {
    if (!explanation || !explanation.explanations) return null;
    const text = explanation.explanations;
  ...
  return (
    <div className="max-w-7xl mx-auto p-7">
      ...
      {/* Instructions from JSON - enunciado */}
      {data.enunciado && (
            ...
            return (
              <div key={idx} className="space-y-1 pb-6 border-b border-gray-100 last:border-0">
                <div className="flex items-center gap-4 py-2">
                  <span className="font-medium text-gray-700 min-w-[100px] text-base">
